...
function sumOfSquares(num1, num2) {
    var sq1 = num1 * num1, sq2 = num2 * num2;
...
function hasVowels(s) {
    ...
    for(var i = 0; i < s.length; i++) {
        if(vowels.indexOf(s.toLowerCase()[i]) !== -1) {
            count++;
        }
    }
...
function firstToLast(seq) {
    var newSeq = [];
    var seqShift = seq.shift();

    newSeq = seq;
    newSeq.push(seqShift);
...
function onlyPositiveNumbers(seq) {
    ...
    for(var i = 0; i < seq.length; i++) {
        if(isPositive(seq[i])) {
            onlyPositiveSeq.push(seq[i])
        }
    }

    return onlyPositiveSeq;
}
...
function onlyNegativeNumbers(seq) {
    ...
    for(var i = 0; i < seq.length; i++) {
        if(isNegative(seq[i])) {
            onlyNegativeSeq.push(seq[i])
        }
    }

    return onlyNegativeSeq;
}
...
function onlyPositiveEvens(seq) {
    var onlyPositiveEvenSeq = [];

    for(var i = 0; i < seq.length; i++) {
        if(isPositive(seq[i]) && isEven(seq[i])) {
            onlyPositiveEvenSeq.push(seq[i])
        }
    }

    return onlyPositiveEvenSeq;
}
...
function onlyPositiveOdds(seq) {
    var onlyPositiveOddSeq = [];

    for(var i = 0; i < seq.length; i++) {
        if(isPositive(seq[i]) && isOdd(seq[i])) {
            onlyPositiveOddSeq.push(seq[i])
        }
    }

    return onlyPositiveOddSeq;
}
...
function onlyNegativeEvens(seq) {
    var onlyNegativeEvenSeq = [];

    for(var i = 0; i < seq.length; i++) {
        if(isNegative(seq[i]) && isEven(seq[i])) {
            onlyNegativeEvenSeq.push(seq[i])
        }
    }

    return onlyNegativeEvenSeq;
}
...
function onlyNegativeOdds(seq) {
    var onlyNegativeOddSeq = [];

    for(var i = 0; i < seq.length; i++) {
        if(isNegative(seq[i]) && isOdd(seq[i])) {
            onlyNegativeOddSeq.push(seq[i])
        }
    }

    return onlyNegativeOddSeq;
}
...
function shortestString(arr) {
    var arrLength = arr[0].length, shortest = arr[0];
...
function longestString(arr) {
    var arrLength = arr[0].length, longest = arr[0];
...
function getUniqueValues(arr) {
    ...
    for(var i = 0; i < arr.length; i++) {
        if(uniqueSeq.indexOf(arr[i]) < 0) {
            uniqueSeq.push(arr[i]);
        }
    }
...
function lowestPriceBook(arr) {
    ...
    return arr.find(obj => obj.price === lowestPrice);
}
...
function getAverageItemPrice(cart) {
    ...
    return totalPrice / cart.items.length;
}
...
function getAverageSpentPerItem(cart) {
    ...
    return totalPrice / totalNumberOfItems(cart);
}
