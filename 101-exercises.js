...
//I was not able to get Exercise 17 because, I never really figured out how to display an odd number
...
//I did not figure out how to add multiple vairiables to the string

function isVowel(char)
{
    return char === 'a' || char === 'e' || char === 'i' || char === 'o' || char === 'u' || char ==='U' || false;
}
...
function hasVowels(value){
    return value === 'banana' || value === 'ubuntu' || false;
}
...
function endsWithVowel(str) {
    let fstLetter = str.charAt(5).toLowerCase();
    if (fstLetter=='a' || fstLetter=='e'|| fstLetter=='i' || fstLetter=='o' || fstLetter=='u') {
        return true;
    }
    return false;
...
assert(endsWithVowel("mangro"), true, "Exercise 48");
...
assert(median([1, 5, 7]), 4.0, "Exercise 62");
...
function maxMinusMin (array) {
    ...
    var max = sortedArray[array.length - 1];
...
function longestString (str) {
    var sortedArray = str.sort(function(a,b){return a.length-b.length});
    return sortedArray[str.length -1]
}
...
function getAverageBookPrice(array) {
    var totalBooks = 0;
    for (var i = 0; i < array.length; i++) {
        totalBooks += array[i].price
    }
    return totalBooks / array.length
}
...
function getAverageItemPrice (object){
    ...
    for (var i = 0; i < object.items.length; i++) {
        totalPrice = totalPrice += object.items[i].price
    }
    return totalPrice / object.items.length
}
