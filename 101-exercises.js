...
assert(vegetables, ["eggplant", "broccoli", "carrot", "cauliflower", "zucchini"], " fExercise 3");
...
console.log("Sorting the vegetables array.");
vegetables.sort();
console.log(vegetables);
//['broccoli', 'carrot', 'cauliflower', 'eggplant', 'tomato', 'zucchin'];
...
fruits.sort();
console.log(fruits);
fruits.reverse('tomato', 'strawberry', 'mango', 'kiwi', 'guava', 'banana');
...
function addOne(numberToTest) {
    return numberToTest + 1;
    addOne (positiveEvenNumber)
}
...
function startsWithVowel(mango){
    var firstLetter = mango.substring(0,1);
    var vowels = /[aeiou]/gi;
...
function endsWithVowel(mango){
    var wordLength = mango.length -1
    var lastLetter = mango.substring(wordLength);
    var vowels = /[aeiou]/gi;
...
function startsAndEndsWithVowel(ubuntu){
    var firstLetter = ubuntu.substring(0,1);
    var vowels = /[aeiou]/gi;
    return vowels.test(firstLetter);
    var wordLength= ubuntu.length -1
    var lastLetter = ubuntu.substrig(wordLength);
    var vowels = /[aeiou]/gi;
    return vowels.test(lastLetter);
}
...
function first(ubuntu){
    var linuxList = Array.from(ubuntu)
    var first = linuxList.slice(0, 1) [0]
    return first
...
function second(ubuntu){
    var linuxList = Array.from(ubuntu);
    var second = linuxList.slice(1,4)[0];
    return second
}
...
function third(ubuntu){
    if (typeof ubuntu === "string"){ubuntu = ubuntu.split("")}
    return ubuntu[2];
...
// function last([ubuntu]){
//   return ubuntu.lastIndexOf(ubuntu,[-1]);
// }


function last(ubuntu){
    return ubuntu[ubuntu.length -1];
}
...
function secondToLast(ubuntu){
    return ubuntu[ubuntu.length -2];
}
...
function thirdToLast(ubuntu){
    return ubuntu[ubuntu.length -3];
}
...
function firstAndSecond(ubuntu){
    return [ubuntu[0], ubuntu[1]];
}
...
function firstAndLast(ubuntu){
    return[ubuntu[0], ubuntu[ubuntu.length-1]];
}
...
function firstToLast(ubuntu){
    let firstElement = ubuntu.shift();
    ubuntu.push(firstElement);
    return ubuntu;
}
