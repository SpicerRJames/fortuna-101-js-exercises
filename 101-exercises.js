...
function isMultipleOfBothThreeAndFive(x) {
    return (isMultipleOfThree(x) && isMultipleOfFive(x))
}
...
function hasVowels(x) {
    return x.split('').filter(letter => 'aeiou'.includes(letter)).length > 0;
}
...
function countVowels(x) {
    return x.split('').filter(letter => 'aeiou'.includes(letter)).length;
}
...
function endsWithVowel(x) {
    ...
    return vowels.indexOf(lastLetter) !== -1;
}
...
function startsAndEndsWithVowel(x) {
    return startsWithVowel(x) && endsWithVowel(x);
}
...
function sumAll(x) {
    return x.reduce(function (accumulater, currentValue){return accumulater + currentValue});
}
...
function productOfAll(x) {
    return x.reduce(function (accumulater, currentValue){return accumulater * currentValue},1);
}
...
function onlyOddNumbers(nums) {
    return nums.filter(num => isOdd(num));
}
...
function onlyEvenNumbers(nums) {
    return nums.filter(num => isEven(num));
}
...
function onlyPositiveNumbers(nums) {
    return nums.filter(num => isPositive(num));
}
...
function onlyNegativeNumbers(nums) {
    return nums.filter(num => isNegative(num));
}
...
function onlyPositiveEvens(nums) {
    return nums.filter(num => isPositiveEven(num));
}
...
function onlyPositiveOdds(nums) {
    return nums.filter(num => isPositiveOdd(num));
}
...
function onlyNegativeEvens(nums) {
    return nums.filter(num => isNegativeEven(num));
}
...
function onlyNegativeOdds(nums) {
    return nums.filter(num => isNegativeOdd(num));
}
...
function totalOfBookPrices(books) {
    return books.reduce((accumulator, book) => accumulator + book.price,0)
}
...
function highestPriceBook(books) {
    return books.sort((b1,b2)=>b2.price-b1.price)[0];
}
...
function lowestPriceBook(books) {
    return books.sort((b1,b2)=>b1.price-b2.price)[0];
}
...
function getAverageItemPrice(cart) {
    var totalCost = cart.items.reduce((accumulator, item) => accumulator + item.price,0)
    return totalCost / numberOfItemTypes(cart);
}
...
function getAverageSpentPerItem(cart) {
    var totalCost = cart.items.reduce((accumulator, item) => accumulator + (item.price * item.quantity),0);
    return totalCost / totalNumberOfItems(cart);
}
...
function mostSpentOnItem (cart) {
    return cart.items.sort((b1,b2)=>(b2.price * b2.quantity)-(b1.price *b1.quantity))[0];
}
