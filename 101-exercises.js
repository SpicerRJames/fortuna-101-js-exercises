...
function isVowel(val) {
    return ['a','e','i','o','u'].indexOf(val.toLowerCase()) != -1;
}
...
function endsWithVowel(str) {
    var strEnd = str.substr(str.length-1);
    if (strEnd == 'a' || strEnd == 'e' || strEnd == 'i' || strEnd == 'o' || strEnd == 'u') {
        return true;
    } else {
        return false;
    }
...
function firstAndLast(arr) {
    var removedItems = arr.splice(1, arr.length - 2);
    return arr;
}
...
function mean(array) {
    var meanOfArray = (array.reduce(function(a,b) {
        return a + b
    }, 0)) / array.length;
    return meanOfArray;
}
...
function getSmallestNumber(array) {
    var smallestNum = array.sort().shift();
    return smallestNum;
}
...
function countPositives(array) {
    return Math.abs(array.length - countNegatives(array));
}
// function countPositives(array) {
//   var counter = 0;
...
function onlyPositiveEvens(array) {
    return onlyEvenNumbers(onlyPositiveNumbers(array));
}
...
function onlyPositiveOdds(array) {
    return onlyOddNumbers(onlyPositiveNumbers(array));
}
...
function onlyNegativeEvens(array) {
    return onlyNegativeNumbers(onlyEvenNumbers(array));
}
...
function onlyNegativeOdds(array) {
    return onlyNegativeNumbers(onlyOddNumbers(array));
}
...
function getAverageBookPrice(object) {
    return (totalOfBookPrices(object) / object.length);
}
...
function highestPriceBook(object) {
    ...
    for (var i = 0; i <object.length; i++) {
        if (object[i].price > counter.price) {
            counter = object[i];
        }
    }
    return counter;
...
function lowestPriceBook(object) {
    var counter = {"price": Infinity};

    object.forEach(function(book) {
        if (book.price < counter.price) {
            counter = book;
        }
    })
    return counter;
...
function getAverageItemPrice(object) {
    ...
    var averagePrice = itemPriceTotal / numberOfItemTypes(object);
...
function getAverageSpentPerItem(object) {
    ...
    return itemPriceTotal / totalNumberOfItems(object);
}
...
function mostSpentOnItem(object) {
    ...
    object.items.forEach(function(item){
        if (item.price*item.quantity > counter); {
            counter = item.price*item.quantity;
            mostSpentOnItem = item;
        }
    });
    return mostSpentOnItem;
}
