...
vegetables = vegetables.concat(['tomato']);
...
var isMultipleOfFive = function (x) {
    // Your code goes here!
    if (x % 5 === 0) {
        return true;
    }
...
var isMultipleOfBothThreeAndFive = function (x) {
    // Your code goes here!
    if (x % 5 === 0 && x % 3 === 0) {
        return true;
    }
...
function sumAll(input){
    var total =  0;
    //this like of code starts the array at 0, continues the arry as long as it is still less than the array, and increments the array by 1 each interation
    for(var i=0; i<input.length; i++)
    {
        //if the input is not a number, skip it and continue the array.
        if(isNaN(input[i])){
            continue;
        }
        //Add total to the input
...
//Write function called mean with an parameter called "input"

//creat loop that will cycle through all variables in the Array
//1.i=0
//2.i<=input.length
//3.i++

//1. inside the loop, write code that adds up all the numbers in an array
//2. after adding up all the numbers, take the mean of the array
//3. return the mean
...
//Write a function named maxMinusMin
function maxMinusMin(input){
    var maxArr = Math.max.apply(Math, input);
    var minArr = Math.min.apply(Math, input);
...
//write function called product of all
function productOfAll(input){
//write code that goes through each value in the array
    var total = 1;
//write code that multiplies all elements in the array
    for(var i = 0; i < input.length; i++){
        total = total * input[i];
    }
//return result
...
//write a function called onlyOddNumbers
//write a if-else statement that returns only odd numbers
//return results
...
//1. iterat trhough input array
//a. chech each value to check if it is even
//2. if it is even we ant to store that in an array
//
function onlyEvenNumbers(array) {
//array has number has our numbers to evaluate

    var evenNumArray = [];
    array.forEach(function(numToEvaluate){
        // if numToEvaluate is even
        //if it is even, we want to put in evenNumArray
        if(isEven(numToEvaluate)){
            //since it is even we want tp push 'numToEvaluate' to evenNumArray
            evenNumArray.push(numToEvaluate)
        }
    });
    return evenNumArray;
...
//write a function called onlyNegativeNumbers
//creat a variable that is an empty array
//ittereate through element
//if each elemet is negavive, save it in a array
//return the array
...
function onlyNegativeNumbers(array){
    var negativeNumbersArray = [];
    array.forEach(function(thisNum){
        if(isNegative(thisNum)){
            negativeNumbersArray.push(thisNum)
        }
    });
...
//write a function called hasEvens
//create a for loop that will go through the array
//slipt the array into individual sections
//if the array has a even number, return true
//if the array does not have an even number, return false.
...
function countEvens (nums){
    return nums.filter((element) => element%2===0).length
}
...
function hasOdds(nums){
    let element = nums.find((element) => element % 2 !== 0);
...
function countNegatives (nums){
    return nums.filter((element) => element <= -1).length
}
...
function countPositives (nums){
    return nums.filter((element) => element >= 0).length
}
...
function onlyPositiveEvens (nums){
    return nums.filter((element) => element >= 0 && element % 2 === 0);
}
...
function onlyNegativeEvens (input){
    return input.filter((element) => element <= 0 && element % 2 === 0)
}
...
//write function named shortestString
//create a for-loop that will go through out the entire array
//if the element in the array is the shortest
...
function totalOfBookPrices (bookArr){
    var sum = 0;
...
function getAverageBookPrice (bookArr){
    var sum = 0;
    for(let book of bookArr){
        sum += book.price
    }
