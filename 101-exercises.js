...
function highestPriceBook(value){
 ...
 for(var i = 0; i < value.length; i++){
     if(testBook.price < value[i].price){
         testBook = value[i];
     }
 }
 return testBook;
...
function numberOfItemTypes(value){
...
function totalNumberOfItems(value){
    var total = 0;
    for(var i = 0; i < value.items.length; i++){
        total = total + value.items[i].quantity;
    }
    return total;
}
...
function getAverageItemPrice(value){
    var total = 0;
    for(var i = 0; i < value.items.length; i++){
        total = total + value.items[i].price;
    }
    return (total / value.items.length)
}
...
function getAverageSpentPerItem(value) {
    var totPrice = 0;
    var totQuant = 0;
    for (var i = 0; i < value.items.length; i++) {
        totPrice = totPrice + (value.items[i].price * value.items[i].quantity);
        totQuant = totQuant + value.items[i].quantity;
    }
    return (totPrice / totQuant);
}
...
function mostSpentOnItem(value){
    ...
    var testCost = (testItem.price * testItem.quantity);
    for(var i = 0; i < value.items.length; i++){
        if(testCost < (value.items[i].price * value.items[i].quantity)){
            testCost = (value.items[i].price * value.items[i].quantity);
            var costlyItem = value.items[i];
        }
    }
    return costlyItem;
}
