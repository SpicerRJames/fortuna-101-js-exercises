...
function removeVowels(string) {
    ...
    for (var i = 0; i < string.length; i++) {
        if (!vowels.includes(string[i])) {
            arrayNotVowels.push(string.charAt(i));
        }
    }
    return arrayNotVowels.join("");
}
...
// Exercise 48
// Write a function definition named endsWithVowel that takes in string and returns true if the string ends with a vowel

function endsWithVowel(value) {
    value = value.toLowerCase();
    var vowels = ["a", "e", "i", "o", "u"];
    for (var i = 0; i < vowels.length; i++) {
        if (value.charAt(value.length - 1) === vowels[i]) {
            return true;
        }
    }
    return false;
...
function startsAndEndsWithVowel(string) {
    ...
    var vowels = ["a", "e", "i", "o", "u"];
    var endsWithVowel = vowels.includes(string.charAt(string.length - 1));
    var startsWithVowel = vowels.includes(string.charAt(0));
    return endsWithVowel && startsWithVowel;
}
...
function firstAndLast(value) {
    var valueFirst = value[0];
    var valueLast = value[value.length - 1];
...
function mean(inputNums) {
    var sum = 0;
    for (var i = 0; i < inputNums.length; i++) {
        sum = sum + inputNums[i];
    }
    var mean = sum / inputNums.length;
    return mean;
}
...
function median(inputNums) {
    inputNums.sort();
    if (inputNums.length % 2 != 0) {
        var midNum = Math.round(inputNums.length / 2);
        return inputNums[midNum - 1]; // - 1 as array index starts at 0
    } else {
        var lowerMidNum = inputNums.length / 2;
        var evenMedian = (inputNums[lowerMidNum - 1] + inputNums[lowerMidNum]) / 2;
        return evenMedian;
    }
}
...
function maxMinusMin(inputNums) {
    var maxNum = Math.max.apply(Math, inputNums);
    console.log(maxNum);
    var minNum = Math.min.apply(Math, inputNums);
    console.log(minNum);
...
function productOfAll(inputNums) {
    var product = 1;
    for (var i = 0; i < inputNums.length; i++) {
        var product = product * inputNums[i];
    }
    return product;
}
...
function getHighestNumber(inputNums) {
    var highestNum = Math.max.apply(Math, inputNums);
    return highestNum;
}
...
function getSmallestNumber(inputNums) {
    var smallestNum = Math.min.apply(Math, inputNums);
    return smallestNum;
}
...
function onlyOddNumbers(inputNums) {
    var oddNums = [];
    for (var i = 0; i < inputNums.length; i++) {
        if (inputNums[i] % 2 != 0) {
...
function onlyEvenNumbers(inputNums) {
    var evenNums = [];
    for (var i = 0; i < inputNums.length; i++) {
        if (inputNums[i] % 2 === 0) {
            evenNums.push(inputNums[i]);
        } else {
            continue;
        }
    }
...
function onlyPositiveNumbers(inputNums) {
    var posNums = [];
    for (var i = 0; i < inputNums.length; i++) {
        if (inputNums[i] > 0) {
            posNums.push(inputNums[i]);
        } else {
            continue;
        }
    }
...
function onlyNegativeNumbers(inputNums) {
    var negNums = [];
    for (var i = 0; i < inputNums.length; i++) {
        if (inputNums[i] < 0) {
            negNums.push(inputNums[i]);
        } else {
            continue;
        }
    }
...
function countNegatives(inputNums) {
    var count = 0;
    for (var i = 0; i < inputNums.length; i++) {
        if (inputNums[i] < 0) {
            count++;
...
function countPositives(inputNums) {
    var count = 0;
    for (var i = 0; i < inputNums.length; i++) {
        if (inputNums[i] > 0) {
            count++;
...
function onlyPositiveEvens(inputNums) {
    var posEvenNums = [];
    for (var i = 0; i < inputNums.length; i++) {
        if (inputNums[i] > 0 && inputNums[i] % 2 === 0) {
            posEvenNums.push(inputNums[i]);
        } else {
            continue;
        }
    }
...
function onlyPositiveOdds(inputNums) {
    var posOddNums = [];
    for (var i = 0; i < inputNums.length; i++) {
        if (inputNums[i] > 0 && inputNums[i] % 2 !== 0) {
            posOddNums.push(inputNums[i]);
        } else {
            continue;
        }
    }
...
function onlyNegativeEvens(inputNums) {
    var negEvenNums = [];
    for (var i = 0; i < inputNums.length; i++) {
        if (inputNums[i] < 0 && inputNums[i] % 2 === 0) {
            negEvenNums.push(inputNums[i]);
        } else {
            continue;
        }
    }
...
function onlyNegativeOdds(inputNums) {
    var negOddNums = [];
    for (var i = 0; i < inputNums.length; i++) {
        if (inputNums[i] < 0 && inputNums[i] % 2 !== 0) {
            negOddNums.push(inputNums[i]);
        } else {
            continue;
        }
    }
...
function shortestString(inputStrings) {
    inputStrings.sort((a, b) => a.length - b.length);
...
function longestString(inputStrings) {
    inputStrings.sort((a, b) => b.length - a.length);
    return inputStrings[0];
}
...
function getUniqueValues(inputVals) {
    return [...new Set(inputVals)];
}
...
function elementsTimesTwo(nums) {
...
function addOneToArray(nums) {
    return nums.map(number => number + 1);
}
...
function highestPriceBook(array) {
    ...
    var priceIndex = 0;
    for (var i = 0; i < array.length; i++) {
        if (array[i].price > compare) {
            compare = array[i].price;
            priceIndex = i;
        }
    }
    return array[priceIndex];
}
...
function totalNumberOfItems(cart) {
    ...
    for (var i = 0; i < shoppingCart.items.length; i++) {
        totalItems = totalItems + shoppingCart.items[i].quantity;
    }
    return totalItems;
}
