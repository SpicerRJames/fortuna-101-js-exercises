...
function endsWithVowel(x) {
    var lastLetter = x.charAt(x.length - 1);
...
function startsAndEndsWithVowel(x) {
    var lastLetter = x.charAt(x.length - 1);
    var firstLetter = x.charAt(0);
    switch (lastLetter.toUpperCase() && firstLetter.toUpperCase()) {
        case "A":
        case "E":
        case "I":
        case "O":
...
function median(x) {
    var numsLen = x.length;
    var result = 0;
    x.sort();
    if (numsLen % 2 === 0) {
        result = (x[numsLen / 2 - 1] + x[numsLen / 2]) / 2;
    } else {
        result = x[(numsLen - 1) / 2];
    }
    return result;
}
...
function getAverageBookPrice(x) {
    ...
    var totalPrice = 0;
    for (var i = 0; i < len; i++) {
        totalPrice = totalPrice + x[i].price;
    }
...
function highestPriceBook(arr) {
    var order = arr.sort((a, b) => b.price - a.price);
    return order[0];
}
...
function lowestPriceBook(arr) {
    var order = arr.sort((a, b) => a.price - b.price);
    return order[0];
}
...
function totalNumberOfItems(arr) {
    ...
    for (var i = 0; i < toGet.length; i++) {
        var total = toGet[i].quantity + total;
    }
    return total;
}
...
function getAverageItemPrice(arr) {
    ...
    for (var i = 0; i < toGet.length; i++) {
        var total = toGet[i].price + total;
        var average = total / toGet.length;
    }
    return average;
}
...
function getAverageSpentPerItem(arr) {
    ...
    for (var i = 0; i < toGet.length; i++) {
        var cost = toGet[i].price * toGet[i].quantity + cost;
        var totalItems = toGet[i].quantity + totalItems;
    }
    var average = cost / totalItems;
    return average;
}
...
function mostSpentOnItem(arr) {
    ...
    var costList = [];
    for (var i = 0; i < toGet.length; i++) {
        var cost = toGet[i].price * toGet[i].quantity;
        if (itemTotal < cost) {
            var itemTotal = cost;
            var greatest = i;
        }
    }
    return toGet[greatest];
}
