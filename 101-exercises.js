...
function hasVowels(x){
    ...
    else if (x.indexOf("e") >= 0){return true}
    else if (x.indexOf("i") >= 0){return true}
    else if (x.indexOf("o") >= 0){return true}
    else if (x.indexOf("u") >= 0){return true}
    else{return false}
}
...
function median(x){
    x.sort((a,b) => a-b);
    var LowM = Math.floor((x.length-1) /2)
    var HighM = Math.ceil((x.length-1) /2)
    return (x[LowM] + x[HighM]) /2
}
...
function countEvens(x){
    ...
    var answer72 = x.filter(Even72)
    return answer72.length
}
...
function countOdds(x){
    ...
    var answer74 = x.filter(odd)
    return answer74.length
}
...
function countNegatives(x){
    ...
    var answer75 = x.filter(negs)
    return answer75.length
}
...
function countPositives (x){
    ...
    var answer76 = x.filter(pos)
    return answer76.length
}
...
function addIttt(x){ return (x+1)};

function addOneToArray (x){
    return x.map(addIttt);
}
...
function highestPriceBook (x) {
    ...
    for(var y = 1; y < (x.length); ++y){
        if (x[y].price > x[y-1].price) {
            mostCostlyBook = x[y];
        }
    }
    return mostCostlyBook;
}
...
// Exercise 97
// Write a function named numberOfItemTypes that takes in the shopping cart as input and returns
...
function numberOfItemTypes (x) {
    ...
    for(var y = 1; y < (x.items.length); ++y){
        if (x.items[y].title !== x.items[y-1].title) {
            console.log (numberOfUniques += 1);
        }
    }
...
function totalNumberOfItems (x) {
    ...
    for(var y = 0; y < (x.items.length); ++y){
        numberOfItems += x.items[y].quantity;
    }
...
// Exercise 100
// Write a function named getAverageSpentPerItem that takes in the shopping cart and returns the average of summing each item's quanties
//  times that item's price.
...
function getAverageSpentPerItem (x) {
    ...
    var totalQuans = 0;
    for(var y = 0; y < (x.items.length); ++y){
        averagePrice += (x.items[y].price * x.items[y].quantity);
        totalQuans += x.items[y].quantity;
    }
    return averagePrice / totalQuans;
}
...
// Exercise 101
// Write a function named mostSpentOnItem that takes in the shopping cart as input and returns the object associated with the item that
//  has the highest price*quantity.
...
function mostSpentOnItem (x) {
    var mostCostlyThing;
    for(var y = 1; y < (x.items.length); ++y){
        if ((x.items[y].price * x.items[y].quantity) > (x.items[y-1].price * x.items[y-1].quantity)) {
            console.log (mostCostlyThing = x.items[y]);
        }
    }
    return mostCostlyThing;
}
